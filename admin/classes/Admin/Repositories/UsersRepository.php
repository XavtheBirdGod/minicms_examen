...
class UsersRepository
{
    ...
    {
        // We gebruiken de nieuwe kolommen: provider en provider_id
        $sql = "SELECT u.id, u.email, u.name, u.is_active, r.name AS role_name
                FROM users u
                JOIN roles r ON r.id = u.role_id
                WHERE u.provider = :provider 
                AND u.provider_id = :provider_id
                AND u.is_active = 1
                LIMIT 1";
    ...
    public function createExternalUser(string $email, string $name, string $provider, string $externalId, int $roleId = 1): int
    {
        $sql = "INSERT INTO users (email, name, provider, provider_id, role_id, is_active, password_hash)
            VALUES (:email, :name, :provider, :external_id, :role_id, 1, NULL)";

        $stmt = $this->pdo->prepare($sql);

        // This will stop the script if the database has an error
        if (!$stmt->execute([
            'email' => $email,
            'name' => $name,
            'provider' => $provider,
            ...
            die("The database rejected the user!");
        }
    ...
    {
        $sql = "SELECT u.id, u.email, u.name, u.role_id, u.is_active, u.provider, u.provider_id, r.name AS role_name
            FROM users u
            JOIN roles r ON r.id = u.role_id
            WHERE u.id = :id
            LIMIT 1";
